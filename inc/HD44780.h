...
#define HD44780_FLG_DIR_RTL 0x00
#define HD44780_FLG_DIR_LTR 0x02
...
typedef struct
{
    ...
    /*
     * Use 8 physical data lines connected to the controller.
     * true -> 8 bit interface (DB7-DB0) | false -> 4 bit interface (DB7-DB4)
     */
    bool interface_8_bit;
    /*
     * Display one taller line on the display.
     * true -> 1 line | false -> 2 lines
     */
    bool single_line;
    /*
     * Use the 5x10 dots character font.
     * Note that the 5x10 dots font only supports 1 line operation (two_lines = false).
...
typedef struct
{
    ...
    bool enable_blink;
    /*
     * Shift the display when data is entered.
     * true -> shift cursor and display | false -> shift only cursor
     */
    bool shift_display;
    /*
     * Shift the display (depending on the shift_display parameter) and cursor in right to left direction
 ...
 *
 * n: Number of positions to shift. When the value is positive the diplay will shift left to right,
 ...
 * When the character is '\t' 4 spaces will be written.
 */
